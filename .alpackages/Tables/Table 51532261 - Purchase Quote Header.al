...
table 51532261 "Purchase Quote Header"
{
    ...
    fields
    {
        field(1; "Document Type"; Option)
        {
            ...
            OptionCaption = 'Quotation Request,Open Tender,Restricted Tender,Low Value Procurement,Direct Procurement,RFP';
            OptionMembers = "Quotation Request","Open Tender","Restricted Tender","Low Value Procurement","Direct Procurement",RFP;
        }
            ...
            begin
                if Status = Status::Released then
                    if UserSetup.Get(UserId) then begin
                        // if not UserSetup."Adjust Bid Dates" then Error('User does not have permission to modify this date');
                    end;
            end;
        }
...
#pragma warning restore AL0603
        ...
        {
            //TableRelation = "Purchase Header"."No." WHERE(Status = FILTER(Released), DocApprovalType = filter(Requisition));
            ...
            begin
                ...
                if RFQ.Find('-') then begin
                    repeat
                        ...
                        PurchLine."Gen. Prod. Posting Group" := RFQ."Gen. Prod. Posting Group";
                        PurchLine."Gen. Bus. Posting Group" := RFQ."Gen. Bus. Posting Group";
                        PurchLine."VAT Prod. Posting Group" := RFQ."VAT Prod. Posting Group";
                        PurchLine."VAT Bus. Posting Group" := RFQ."VAT Bus. Posting Group";
                        PurchLine.Insert;
                    until RFQ.Next = 0;
                end;
            end;
        }
        field(39005557; "Evaluation Committee"; code[20])
        {
            TableRelation = "Procurement Committees" where("Committee Type" = filter(Evaluation));
        }
        field(39005558; "Procurement Methods"; code[20])
        {
            TableRelation = "Procurement Methods";
        }
        field(39005559; Published; Boolean)
        {
    ...
    begin
        ...
        if "No." = '' then begin
            ...
            IF "Document Type" = "Document Type"::"Quotation Request" THEN BEGIN
                ...
                //NoSeriesMgt.InitSeries(PurchSetup."Quotation Request No", xRec."No. Series", 0D, "No.", "No. Series");
            END
            ELSE
