table 51532206 "Payment Line"
{
    ...
    fields
    {
        ...
        field(3; Type; Code[20])
        {
            ...
            begin
                ...
                if RecPayTypes.Find('-') then begin
                    ...
                    "IsMedical Claim" := RecPayTypes."Is Medical Claim";
        ...
        field(13; "Account No."; Code[20])
        {
            ...
            begin
                ...
                PH.Get(No);
                "Account Name" := '';
                "External Clearance Entry" := 0;
                "External Clearance Loan No." := '';
                RecPayTypes.Reset;
                RecPayTypes.SetRange(RecPayTypes.Code, Type);
                RecPayTypes.SetRange(RecPayTypes.Type, RecPayTypes.Type::Payment);
                ...
                then
                    ...
                    "Account Type"::Credit:
                      begin
                        ...
                        CreditAccounts.CalcFields("Balance (LCY)");
                        "Account Name":=CreditAccounts.Name;
                        "Account Balance":=CreditAccounts."Balance (LCY)";
                      end;
                    end;
        ...
        field(20; Amount; Decimal)
        {
            ...
            begin
                ...
                /*if PH.Get(No) then begin
                  if PH."Paying Account Type"  PH."Paying Account Type"::"G/L Account" then begin
                    if "Account Type" = "Account Type"::Savings then begin
                        SavingsAccounts.Get("Account No.");
                        if Amount > Banking.GetAccountBalance("Account No.") then
                          Error('Will Lead to Overdrawal. Current Balance is %1',Banking.GetAccountBalance("Account No.") );
                    end;
                  end;
                end;
                ...
                if ("Transaction Type"="Transaction Type"::Loan) or ("Transaction Type"="Transaction Type"::Repayment) then begin
                  if Amount>Loans."Outstanding Balance"*-1 then Amount:=Loans."Outstanding Balance"*-1  else Amount:=Amount;
                  end;

                if ("Transaction Type"="Transaction Type"::"Interest Due") or ("Transaction Type"="Transaction Type"::"Interest Paid") then begin
                  if Amount>Loans."Outstanding Interest"*-1 then Amount:=Loans."Outstanding Interest"-1  else Amount:=Amount;
                  end;
                ...
                Loans.SetRange("Loan No.",Rec."Loan No");
                if Loans.Find('-') then begin
                    Loans.CalcFields("Outstanding Balance","Outstanding Interest","Outstanding Loan Reg. Fee","Outstanding Partial");

                    if "Transaction Type" = "Transaction Type"::"Partial Disbursement" then begin
                        if Amount > "Account Balance" then
                            Error('Amount cannot Exceed Balance');
                    end;

                end;

                if "Account Payment Type"  "Account Payment Type"::Normal then
                  if "Account Type"="Account Type"::Savings then begin
                    TestField("Account Payment Type");
                    end;*/
            end;
        }
        ...
        field(26; "Withholding Tax Amount"; Decimal)
        {
            ...
            begin
                ...
                */
                ...
                "Net Amount" := Amount - "VAT Withholding Amount" - "Withholding Tax Amount" - "Retention  Amount";
        ...
        field(56006; "Payment Account"; Code[20])
        {
            //TableRelation = IF ("Pay Mode"=CONST("Fosa Account")) "Savings Accounts";

            trigger OnValidate()
            begin
                /**"Payment Account Name":='';
                if SavingsAccounts.Get("Payment Account") then;
                "Payment Account Name":=SavingsAccounts.Name;**/
            end;
        }
        field(56007; "Payment Account Name"; Text[100])
        {
            Editable = false;
        }
        field(56008; "Tax Exempt"; Decimal)
        {
        }
        field(56009; "Taxable Amount"; Decimal)
        {
        }
        field(56010; "Total Tax Exemption"; Decimal)
        {
        ...
        field(56011; "Staff No."; Code[20])
        {
            //  TableRelation = "HR Employees";
        }
        field(56012; "External Clearance Loan No."; Code[20])
        {
            DataClassification = ToBeClassified;
            // TableRelation = Loans WHERE ("External Clearance Pending"=FILTER(>0));
        }
        field(56013; "External Clearance Entry"; BigInteger)
        {
            DataClassification = ToBeClassified;
            /**TableRelation = "Other Commitements Clearance"."Entry No." WHERE ("Loan No."=FIELD("External Clearance Loan No."),
                                                                               "PV Posted"=CONST(false),
                                                                               "Account No."=FIELD("Account No."));**/

            trigger OnValidate()
            begin
                /**if "External Clearance Entry"  0 then begin

                    OtherCommitementsClearance.Get("External Clearance Loan No.","External Clearance Entry");
                    Validate("Account Name",OtherCommitementsClearance.Description);
                    Validate(Amount,OtherCommitementsClearance.Amount);

                end;**/
            end;
        }
        field(56014; "Account Balance"; Decimal)
        {
            DataClassification = ToBeClassified;
        }
        field(56015; "Transaction Type"; Option)
        {
            DataClassification = ToBeClassified;
            OptionCaption = ' ,Loan,Repayment,Interest Due,Interest Paid,Bills,Appraisal Due,Loan Registration Fee,Appraisal Paid,Pre-Earned Interest,Penalty Due,Penalty Paid,Partial Disbursement';
            OptionMembers = " ",Loan,Repayment,"Interest Due","Interest Paid",Bills,"Appraisal Due","Loan Registration Fee","Appraisal Paid","Pre-Earned Interest","Penalty Due","Penalty Paid","Partial Disbursement";
            ValuesAllowed = " ", Repayment, "Interest Paid", "Loan Registration Fee", "Appraisal Paid", "Penalty Paid", "Partial Disbursement";
            ...
            begin
                ...
                if Loans.Find('-') then begin
                    ...
                    if ("Transaction Type"="Transaction Type"::Loan) or ("Transaction Type"="Transaction Type"::Repayment) then begin
                        if Amount>Loans."Outstanding Balance" then Amount:=Loans."Outstanding Balance"  else Amount:=Amount;
                    end;

                    if ("Transaction Type"="Transaction Type"::"Interest Due") or ("Transaction Type"="Transaction Type"::"Interest Paid") then begin
                        if Amount>Loans."Outstanding Interest" then Amount:=Loans."Outstanding Interest"  else Amount:=Amount;
                    end;
                    ...
                    if "Transaction Type" = "Transaction Type"::"Partial Disbursement" then
                      "Account Balance" := Loans."Outstanding Partial"*-1;

                    Amount := "Account Balance";
                    "Net Amount":="Account Balance";
                    "NetAmount LCY":="Account Balance";
                end;**/
            end;
        }
        field(56016; "Account Payment Type"; Option)
        {
            DataClassification = ToBeClassified;
            OptionCaption = ' ,Deposit Refund,Interest Earned,Normal';
            OptionMembers = " ","Deposit Refund","Interest Earned",Normal;
                ...
                case "Account Type" of
                  "Account Type"::Savings:
                    begin
                      if "Account No." '' then
                      SavingsAccounts.Get("Account No.");
                      SavingsAccounts.CalcFields("Interest Balance","Principle Balance");
                      "Account Name":=SavingsAccounts.Name;
                      "Account Balance":=SavingsAccounts."Balance (LCY)";
                      if "Account Payment Type"="Account Payment Type"::"Deposit Refund" then begin
                         "Account Balance":=SavingsAccounts."Principle Balance" end else
                      if "Account Payment Type"="Account Payment Type"::"Interest Earned" then begin
                         "Account Balance":=SavingsAccounts."Interest Balance";
                        end else "Account Balance":=0;
                    end;
                  end;**/
            end;
        }
        field(56017; "IsMedical Claim"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(56018; "Medical Claim No"; Code[20])
        {
            DataClassification = ToBeClassified;
            //TableRelation = "HR Medical Claims" WHERE ("Payment Status"=FILTER(<>Paid));

            trigger OnValidate()
            begin
                /**TestField("IsMedical Claim");
                PayLine.Reset;
                PayLine.SetRange("Medical Claim No",Rec."Medical Claim No");
                if PayLine.Find('-') then Error('This Claim Can not be paid twice');

                HRMedicalClaims.Reset;
                HRMedicalClaims.SetRange("Claim No","Medical Claim No");
                HRMedicalClaims.SetFilter("Payment Status",'%1',HRMedicalClaims."Payment Status"::" ");
                if HRMedicalClaims.Find('-') then begin
                  Amount:=(HRMedicalClaims."Outpatient Amount Charged"+HRMedicalClaims."Inpatient Amount Charged");
                  "Taxable Amount":=Amount;
                  "Net Amount":=Amount;
                  "NetAmount LCY":=Amount;
                end;**/
            end;
        }
        field(56019; Narration; Text[150])
        {
            DataClassification = ToBeClassified;
        }
    }
    ...
    var
        ...
        Banking: Codeunit MobileBanking;
    ...
    begin
        ...
        if "Taxable Amount" <> 0 then begin
            ...
            if "VAT Withholding Rate" <> 0 then begin
                //"VAT Withholding Amount" := TaxCalc.CalculateTax(Rec, CalculationType::"W/VAT");
